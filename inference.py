...
class ExactInference(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.beliefs.normalize() #makes it equal to 1
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined() # take out this line to see what the code below does
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        repeats =self.numParticles/len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        # case 1
        if noisyDistance == None:
            prisionPosition=self.getJailPosition()
            for p in range(len(self.listOfParticles)):
                self.listOfParticles[p]= prisionPosition
        else:
            for particle in self.listOfParticles:
                allPos[particle] += emissionModel[util.manhattanDistance(particle, pacmanPosition)]
            if allPos.totalCount() == 0: # case 2
                self.initializeUniformly(gameState) 
            else:
                if allPos.totalCount() == 0:
                    ...
                    for i in range(self.numParticles):
                        self.listOfParticles.append(util.sample(allPos))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        listPosCartesianProduct = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        #print listPosCartesianProduct
        self.listOfParticlesJ=[]
        repeats= self.numParticles/len(listPosCartesianProduct)
        for position in listPosCartesianProduct :
            for time in range(repeats):
                self.listOfParticlesJ.append(position)
    ...
    def observeState(self, gameState):
        ...
        for p in  range(len(self.listOfParticlesJ)): 
            particleProb=1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                   self.listOfParticlesJ[p] =self.getParticleWithGhostInJail(self.listOfParticlesJ[p], i)
                else:
                    particleProb*=emissionModels[i][util.manhattanDistance(self.listOfParticlesJ[p][i], pacmanPosition)]
            allPossible[self.listOfParticlesJ[p]] += particleProb
        #All 0
        if allPossible.totalCount() == 0:
            ...
            for p in  range(len(self.listOfParticlesJ)): 
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None:
                        self.getParticleWithGhostInJail(self.listOfParticlesJ[p], i)
        else: 
            self.listOfParticlesJ=[]
            ...
            for i in range(self.numParticles):
                self.listOfParticlesJ.append(util.sample(allPossible))
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.listOfParticlesJ:
        ...
        self.listOfParticlesJ = newParticles

    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        beliefsDis = util.Counter()
        for particle in self.listOfParticlesJ:
            beliefsDis[particle] += 1
        beliefsDis.normalize()
        return beliefsDis
